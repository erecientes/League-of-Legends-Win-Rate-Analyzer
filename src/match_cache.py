...
class MatchCache:
        ...
        with self._connect() as conn:
            conn.execute("""
                CREATE TABLE IF NOT EXISTS matches (
                    ...
                    match_data  TEXT NOT NULL,   -- raw JSON blob from Riot API
    ...
    def save_match(self, match_id: str, match_data: dict) -> None:
        with self._connect() as conn:
            conn.execute(
                "INSERT OR IGNORE INTO matches (match_id, match_data) VALUES (?, ?)",
    ...
    # Generator to iterate over all cached matches. Yields (match_id, match_data) tuples.
    def iter_matches(self):
        with self._connect() as conn:
            cursor = conn.execute("SELECT match_id, match_data FROM matches")
            for match_id, raw in cursor:
                yield match_id, json.loads(raw)
